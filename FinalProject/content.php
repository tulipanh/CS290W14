...
<head>
	...
	* {padding:0;margin:0;}
	...
	.itempic {height:500px;max-width:auto;margin: 10px 60px 10px 60px;}
...
if (!($stmt = $mysqli->prepare("SELECT title, date FROM submission WHERE userid = " . $vuid . " AND active = 1"))) {
    echo '<script> document.getElementById("error").innerHTML = "Prepare failed: (' . $mysqli->errno . ')";</script>';
}
...
if (!$stmt->bind_result($title, $date)) {
    echo '<script> document.getElementById("error").innerHTML = "Binding output parameters failed: (' . $stmt->errno . ')"</script>';
}
while($stmt->fetch());
echo $title;
echo '</h2>';
echo '<div><a class="itempic" href="http://web.engr.oregonstate.edu/~tulipanh/Final/images/' . $vuid . '-' . $vusub . '-1.png"><img class="itempic" src="./images/' . $vuid . '-' . $vusub . '-1.png"></a></div>';
echo '<p>';
$filestring = './subtext/' . $vuid . '-' . $vusub . '.txt';
$file = file_get_contents($filestring, true);
echo $file;
echo '</p>';
?>
					</td></tr></table>
